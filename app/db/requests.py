...
async def set_user(username, chat_id):
    async with async_session() as session:
        user = await session.scalar(select(User).where(User.username == username).where(User.chat_id == chat_id))
...
async def set_user_available(username, chat_id, available, date_to_available=None):
    async with async_session() as session:
        stmt = update(User).where(User.username == username).where(User.chat_id == chat_id)
...
async def get_active_users(chat_id):
    async with async_session() as session:
        stmt = select(User.username).where(User.available).where(User.chat_id == chat_id)
